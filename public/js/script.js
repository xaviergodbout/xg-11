(function () {
  ...
  function updateThemeFavicon() {
    ...
    const faviconHref = isDarkMode ? '/img/x-g_favicon_dark.svg' : '/img/x-g_favicon_light.svg';
  ...
  function watchThemeFavicon() {
    const colorSchemeQuery = window.matchMedia('(prefers-color-scheme: dark)');

    if (typeof colorSchemeQuery.addEventListener === 'function') {
      colorSchemeQuery.addEventListener('change', updateThemeFavicon);
      return;
    }

    if (typeof colorSchemeQuery.addListener === 'function') {
      colorSchemeQuery.addListener(updateThemeFavicon);
    }
  }
